...
@if($type =='link')
<a href="{{ $href }}" target="{{ $target }}" @class(['btn', 'btn-'.$color => !empty($color), 'w-100' => $fullwidth, 'active' => $active, 'disabled' => $disabled, 'btn-icon' => $icononly]) @if(!empty($arialable)) {{ $attributes->merge(['aria-label' => $arialabel]) }} @endif>
    {{ $slot }}
</a>
@else
