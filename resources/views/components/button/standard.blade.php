...
@if($type == 'link')
<a href="{{ $href }}" target="{{ $target }}" {{ $attributes->class(['btn', 'btn-'.$color => !empty($color), 'w-100' => $fullwidth, 'active' => $active, 'disabled' => $disabled, 'btn-icon' => $icononly]) }}>
    {{ $slot }}
</a>
@else
<button {{ $attributes->class(['btn', 'btn-'.$color => !empty($color), 'w-100' => $fullwidth, 'active' => $active, 'disabled' => $disabled, 'btn-icon' => $icononly])->merge([ "type" => $type ]) }}>
    {{ $slot }}
</button>
@endif